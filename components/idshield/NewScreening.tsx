...
export function NewScreening({open,onClose,native,onSample,onCreated,notify}:{open:boolean;onClose:()=>void;native:boolean;onSample:(i:number)=>Promise<void>;onCreated:(id:string)=>void;notify:(s:string)=>void}){
 ...
 async function submit(){
  ...
  try{
   if(mode==='sample')await onSample(sample)
    ...
    if(files.length<2)throw new Error('Upload or capture at least two documents so the engine can compare them.')
 ...
 return <Dialog open={open} onOpenChange={value=>!value&&onClose()}><DialogContent className="intake-dialog"><DialogHeader><div className="eyebrow">STAGE 1 · INTAKE</div><DialogTitle>Start a real multi-document screening</DialogTitle><DialogDescription>Upload two to four consented ID images. Use an original and a suspected edited copy, or two proofs for the same person. Every result is computed from the supplied pixels.</DialogDescription></DialogHeader>
  ...
  {mode==='sample'?<><div className="sample-grid">{samples.map(s=><button key={s.i} className={'sample-card '+(sample===s.i?'selected':'')} onClick={()=>setSample(s.i)}><span className={'risk-badge '+s.kind}>{s.kind} risk scenario</span><strong>{s.name}</strong><small>{s.detail}</small>{sample===s.i&&<Check size={16}/>}</button>)}</div><div className="module-note"><Info size={15}/>{native?'Runs local OCR and CV on a newly generated fictional specimen.':'Browser companion replays a clearly labeled fixture. Use the Windows launcher for actual OCR and CV.'}</div></>:<>
   ...
   <div className="upload-dropzone" role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' ')fileRef.current?.click()}} onClick={()=>fileRef.current?.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();addFiles(Array.from(e.dataTransfer.files))}}><UploadCloud/><strong>Choose at least two document images or PDFs</strong><span>Document 1 can be the reference; Document 2 can be the suspected edit · 8 MB each · up to 4</span><input ref={fileRef} type="file" accept="image/png,image/jpeg,application/pdf" multiple hidden onChange={e=>addFiles(Array.from(e.target.files||[]))}/></div>
   <Button variant="outline" disabled={!native||!consent} onClick={()=>startCamera('document')}><Camera/>Capture document with camera</Button>
   {files.map((f,i)=><div className="upload-file" key={`${f.name}-${i}`}><FileText size={16}/><span><strong>Document {i+1}</strong> · {f.name}</span><small>{(f.size/1024).toFixed(0)} KB</small><Button variant="ghost" size="icon-sm" aria-label={'Remove '+f.name} onClick={()=>setFiles(files.filter((_,n)=>n!==i))}><Trash2/></Button></div>)}
  ...
  <Button className="primary-button intake-submit" disabled={busy||(mode==='upload'&&(!native||!consent||files.length<2))} onClick={submit}>{busy?<LoaderCircle className="spin"/>:mode==='sample'?<Play/>:<ShieldCheck/>}{mode==='sample'?'Run seeded scenario':'Process documents live in this session'}</Button>
